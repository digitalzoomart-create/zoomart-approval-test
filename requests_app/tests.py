...
class BaseWorkflowTestCase(TestCase):
    def setUp(self):
        for name in ["Employee", "Manager", "Finance", "Administrator", "Senior Management"]:
        ...
        mid = ApprovalWorkflowRule.objects.create(name="Mid", min_amount=500.01, max_amount=2000)
        ApprovalStepRule.objects.create(workflow_rule=mid, order=1, approver_role=ApprovalStepRule.ROLE_DEPARTMENT_MANAGER)
        ApprovalStepRule.objects.create(workflow_rule=mid, order=2, approver_role=ApprovalStepRule.ROLE_FINANCE)

        high = ApprovalWorkflowRule.objects.create(name="High", min_amount=2000.01, max_amount=None)
        ApprovalStepRule.objects.create(workflow_rule=high, order=1, approver_role=ApprovalStepRule.ROLE_DEPARTMENT_MANAGER)
        ApprovalStepRule.objects.create(workflow_rule=high, order=2, approver_role=ApprovalStepRule.ROLE_FINANCE)
        ApprovalStepRule.objects.create(workflow_rule=high, order=3, approver_role=ApprovalStepRule.ROLE_SENIOR_MANAGER)
...
class ApprovalRoutingTests(BaseWorkflowTestCase):
    ...
    def test_mid_amount_needs_manager_then_finance(self):
        ...
        services.approve(req, self.manager_a, "ok")
        req.refresh_from_db()
        self.assertEqual(req.status, Request.STATUS_PENDING_FINANCE, "must advance to Finance next, not skip it")
...
class PendingForUserTests(BaseWorkflowTestCase):
    ...
    def test_future_senior_step_does_not_appear_before_finance_has_acted(self):
        req = self._make_request(self.employee_a, self.dept_a, 5000)  # needs Manager -> Finance -> Senior
        services.submit_request(req, self.employee_a)
        services.approve(req, self.manager_a, "ok")
        req.refresh_from_db()
        self.assertEqual(req.status, Request.STATUS_PENDING_FINANCE)
        ...
        self.assertNotIn(
            ...
            "the Senior Manager step exists in the DB already but is not active yet, "
            "so it must not appear in the director's approval queue while Finance is still pending",
        )
